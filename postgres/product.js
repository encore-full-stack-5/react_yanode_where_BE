...
exports.getProductByORD_ID = (gds_id) =>{
    return pg.getQuery("SELECT * FROM order where \"GDS_ID\" = $1", [gds_id]);
}

//비번, useyn등 필요없는거 제외하는 수정필요
/**
 * `Product`Table에서 매장ID가 일치하는 상품의 목록을 반환합니다.
 * @param {*} shop_id - 매장 이 름
...
exports.getProductBySHOP_IDAndIsEXPSR_YN = (shop_id) =>{
    return pg.getQuery('SELECT * FROM product where "EXPSR_YN" = 1 and "SHOP_ID"=$1', [shop_id])
}
...
exports.updateProductYNByGDS_ID = (props) =>{
    return pg.updateQuery("", [...props])
}
